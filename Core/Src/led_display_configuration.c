...
void display7SEG2(int num) {
	uint8_t the_number = sevenSegmentLEDConversion[num];
	if(the_number & 0x01) {
		HAL_GPIO_WritePin(LED_2_G_GPIO_Port, LED_2_G_Pin, 1);
	} else
		HAL_GPIO_WritePin(LED_2_G_GPIO_Port, LED_2_G_Pin, 0);

	if(the_number & 0x02) {
			HAL_GPIO_WritePin(LED_2_F_GPIO_Port, LED_2_F_Pin, 1);
		} else
			HAL_GPIO_WritePin(LED_2_F_GPIO_Port, LED_2_F_Pin, 0);

	if(the_number & 0x04) {
			HAL_GPIO_WritePin(LED_2_E_GPIO_Port, LED_2_E_Pin, 1);
		} else
			HAL_GPIO_WritePin(LED_2_E_GPIO_Port, LED_2_E_Pin, 0);

	if(the_number & 0x08) {
			HAL_GPIO_WritePin(LED_2_D_GPIO_Port, LED_2_D_Pin, 1);
		} else
			HAL_GPIO_WritePin(LED_2_D_GPIO_Port, LED_2_D_Pin, 0);

	if(the_number & 0x10) {
			HAL_GPIO_WritePin(LED_2_C_GPIO_Port, LED_2_C_Pin, 1);
		} else
			HAL_GPIO_WritePin(LED_2_C_GPIO_Port, LED_2_C_Pin, 0);

	if(the_number & 0x20) {
			HAL_GPIO_WritePin(LED_2_B_GPIO_Port, LED_2_B_Pin, 1);
		} else
			HAL_GPIO_WritePin(LED_2_B_GPIO_Port, LED_2_B_Pin, 0);

	if(the_number & 0x40) {
			HAL_GPIO_WritePin(LED_2_A_GPIO_Port, LED_2_A_Pin, 1);
		} else
			HAL_GPIO_WritePin(LED_2_A_GPIO_Port, LED_2_A_Pin, 0);
